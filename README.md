...
See the [![pub](https://img.shields.io/pub/v/image_compress_plus.svg)](https://pub.dev/packages/image_compress_plus/versions) version.

```yaml
dependencies:
  image_compress_plus: <latest_version>
```

or run this command:

```bash
flutter pub add image_compress_plus
```
...
[See full example](https://github.com/WuTangNaiLao/image_compress_plus/blob/main/packages/image_compress_plus/example/lib/main.dart)
...
```dart
  ...
  Future<Uint8List> testCompressFile(File file) async {
    var result = await ImageCompressPlus.compressWithFile(
      file.absolute.path,
      minWidth: 2300,
      minHeight: 1500,
      quality: 94,
  ...
  Future<File> testCompressAndGetFile(File file, String targetPath) async {
    var result = await ImageCompressPlus.compressAndGetFile(
  ...
  Future<Uint8List> testCompressAsset(String assetName) async {
    var list = await ImageCompressPlus.compressAssetImage(
      assetName,
      minHeight: 1920,
      minWidth: 1080,
      quality: 96,
  ...
  Future<Uint8List> testComporessList(Uint8List list) async {
    var result = await ImageCompressPlus.compressWithList(
      list,
      minHeight: 1920,
      minWidth: 1080,
      quality: 96,
...
Detailed examples are available in `migrate.md`.
